...
| 33 | Apr 23 | Two Sum (LC 1) | Hashmap/Set (complement lookup) | Correct | 1 | Easy | Complement lookup instant. Check then store order correct. Clean code. | May 7 |
| 34 | Apr 23 | Maximum Subarray (LC 53) | Kadane's | Correct | 2 | Good | Pattern correct. Invariant initially wrong (< 0 reset instead of max choice). += vs = typo. Core Kadane decision concept clarified. | Apr 26 |
| 35 | Apr 23 | Move Zeroes (LC 283) | Two Pointers (slow-fast) | Correct | 1 | Good | Pattern correct (two pointers). Direction wrong initially (opposite ends instead of same direction). After correction, clean code. | Apr 26 |
| 36 | Apr 23 | Best Time to Buy and Sell Stock (LC 121) | Kadane's (min so far) | Correct | 3 | Good | Pattern correct. Condition ulti (> instead of <), left increment instead of direct jump, while loop attempt. Core: if smaller found, left = right directly. | Apr 26 |
| 37 | Apr 25 | Trapping Rain Water (LC 42) — RETRY 2 | Monotonic Stack | Correct | 1 | Good | Pattern correct. Initial recall had wrong direction (pop when smaller instead of bigger). After correction, formula perfect: min(left,right)-bottom, width=right-left-1. Empty stack check correct. Clean code. | Apr 28 |
| 38 | Apr 25 | Maximum Sum Circular Subarray (LC 918) — RETRY 2 | Kadane's | Correct | 1 | Easy | Pattern, approach, edge case (all negative) — all correct. Minor: initial values could be 0 instead of inf, total calculable inside loop, unused variable. No help needed. | May 9 |
